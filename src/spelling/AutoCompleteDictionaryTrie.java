...
public class AutoCompleteDictionaryTrie implements  Dictionary, AutoComplete {
	...
	public boolean addWord(String word) {
		...
		for (int i = 0; i < wordArray.length; i++) {
			if (!curr.getValidNextCharacters().contains(wordArray[i])) {
	...
	{
	    // TODO: Implement this method
		String wordLCase = s.toLowerCase();
		TrieNode curr = root;
		char[] wordArray = wordLCase.toCharArray();
...
//			 	return Collections.<String> emptyList();
		 ...
		 while (!queue.isEmpty()){
			 ...
			 TrieNode trieNode = queue.remove();
			 if (trieNode.endsWord())
			 	completions.add(trieNode.getText());
			 for (char c: trieNode.getValidNextCharacters())
			 	queue.add(trieNode.getChild(c));
		 }
