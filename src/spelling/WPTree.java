...
public class WPTree implements WordPath {
	...
	public List<String> findPath(String word1, String word2) 
	{
        ...
        while (!queue.isEmpty()){
            ...
            List<String> realWords = nw.distanceOne(curr.getWord(), true);
            for (String w: realWords) {
                if (!visited.contains(w)){
                    curr.addChild(w);
                    visited.add(w);
                    WPTreeNode childNode = new WPTreeNode(w, curr);
                    queue.add(childNode);
                    if (childNode.getWord().equals(word2)) {
